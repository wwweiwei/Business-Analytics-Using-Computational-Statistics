...
bootstrap_null_alt <- function(sample0, sample1) { 
  ...
  resample_se_1 <- sd(resample) / sqrt(length(resample))
  
  t_stat_alt <- (mean(resample) - mean(resample_1)) / resample_se
...
plot(density(t_alt), main = "bootstrap density plot", lwd=2, ylim=c(0,0.45)) 
