...
missing: **scree plot of simulated noise**
...
- **Parallel Analysis**: Parallel analyis is an alternative technique that compares the scree of factors of the observed data with that of a random data matrix of the same size as the original. 
...
# Question 2 Earlier, we examined the eigenvectors of the security dataset. Now, let’s examine factor loadings (use the principal() method from the psych package)
...
```
- Answer: <br>
  - Loadings, which include magnitude and direction are easier to interpret than eigenvectors. λ > 0.70 is considered a good loading, more than half of item variance explained by PC. <br>
  - As a result, PC1 belongs to Q1, Q14, Q18. <br>
...
```{r }
dec_pca3_rotate <- principal(security_questions, nfactor=3, rotate="nane", scores=TRUE)
