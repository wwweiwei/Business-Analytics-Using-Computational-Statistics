...
### Question 1) Let’s reexamine what it means to standardize data. To standardize a vector, 
...
```{r fig.width = 5, fig.height = 3.7}
rnorm <- rnorm(n=100000, mean=940, sd=190)
...
print("mean:")
print(mean(rnorm_std))
print("sd:")
print(sd(rnorm_std))
```
- Answer: After standardized, the mean should be 0 and the sd should be 1.
<br>
...
$$
- Answer: Every normal distribution is a version of the standard normal distribution. if X is a normal deviate with parameters \mu and \sigma ^{2}, then this X distribution can be re-scaled and shifted via the formula Z = (X−\mu)/\sigma
...
print("Original bookings datetime:")
bookings$datetime[1:10]
print("Standardized version (minday_std):")
minday_std[1:10]
```
<br>
...
sd(minday)
print("95% CI:")
quantile(minday, probs=c(0.025, 0.975))
...
resamples
#print("resamples_mean:")
#print(mean(resamples))
```
<br>
